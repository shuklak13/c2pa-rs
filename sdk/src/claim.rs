...
use crate::{
    ...
    jumbf::{
        ...
        labels::{
            assertion_label_from_uri, box_name_from_uri, manifest_label_from_uri, to_databox_uri,
            uri_is_relative, ASSERTIONS, CREDENTIALS, DATABOX, DATABOXES, SIGNATURE,
        },
    },
...
impl Claim {
    ...
    fn redact_assertion_from_self(&mut self, assertion_uri: &str) -> Result<()> {
    ...
    fn verify_internal(
        ...
        for assertion in claim.assertions() {
            ...
            // is this assertion in the incoming redaction list, skip if one of ours
    ...
    pub(crate) fn add_ingredient_data(
        ...
        if let Some(redactions) = &redactions_opt {
            for redaction in redactions {
                if let Some(claims) = self.ingredients_store.get_mut(provenance_label) {
                    if let Some(claim) = claims.iter_mut().find(|x| redaction.contains(x.label())) {
                        claim.redact_assertion_from_self(redaction)?;
                    } else {
                    }
                } else {
        ...
        if let Some(curr_redactions) = &mut self.redacted_assertions {
            if let Some(mut ingredient_redactions) = redactions_opt {
                curr_redactions.append(&mut ingredient_redactions);
            }
        } else {
